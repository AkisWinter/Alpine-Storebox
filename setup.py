...
def remove_duplicates(users):
    ...
    for user in users:
        ...
        if (
            ...
            and (user["userid"] not in seen_userids)
            and (user["groupid"] not in seen_groupids)
            ...
            seen_groupids.add(user["groupid"])
...
def create_user(username, userid, sshkeys):
    ...
    If the user does not exist, it creates the user with the specified username, userid,
    and groupid.
    If the user already exists, the function does nothing.
    After creating the user, the function creates a .ssh directory in the user's home directory
    and sets the correct permissions.
    It then adds the provided SSH keys to the user's authorized_keys file.

    Note:
    This function assumes that the 'useradd', 'mkdir', 'chown', 'chmod', and 'touch' commands are
...
def generate_ssh_host_keys():
    ...
    This function generates SSH host keys for the following key types: RSA, ECDSA, and Ed25519.
...
def copy_ssh_default_config():
    ...
    This function copies the default SSH configuration from '/defaults/ssh/' to '/etc/ssh/'.
